...
//
// master stealth switch â€?set to 0 to disable all stealth globally
//
...
#define SHADOW_PT_MODE      SHADOW_PT_MODE_A
...
// the VM-exit overhead, then RDTSC exiting is disabled. zero drift â€?
...
// disabled by default â€?enable once base hv is verified stable
//
...
//
// SEH note: windows SEH cannot work in VMX-root mode â€?we use a private
...
//
// CR4.VMXE â€?bit 13 (may already be defined in ia32.h)
//
...
//
// CR4.LAM_SUP â€?bit 28 (supervisor linear address masking)
...
// used only for the old accumulating approach â€?no longer needed
//
...
typedef struct _STEALTH_CPUID_CACHE {
    ...
    // if forced, we can't toggle it off â€?just use the armed flag.
    //
