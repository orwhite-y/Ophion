            ...
            if (fn && TdAsciiEqualI(fn, func_name))
            {
                ...
                if (ord < exp_dir->NumberOfFunctions)
                {
                    ...
                    if (func_rva >= exp_rva && func_rva < exp_rva + exp_sz)
                        return NULL;  // forwarded export 闁?skip
...
//
// R3 EPT hook params 闁?from user-mode app via DeviceIoControl
//
...
//
// flow:
//   PEB 闁?kernel32 base 闁?parse exports 闁?find GetProcAddress (hash-based)
//   GetProcAddress(kernel32, "LoadLibraryA") 闁?LoadLibraryA("user32.dll")
//   GetProcAddress(user32, "MessageBoxA") 闁?MessageBoxA(0, text, title, 0)
//   ret
//
...
//
//   ; --- find_export(rbx=base, r12d=hash) 闁?rax=funcVA ---
...
//
//   (see byte array below 闁?hand-assembled and verified)
//
...
// zero runtime API calls 闁?all resolution done here at PASSIVE_LEVEL.
...
// =========================================================================
//  DPC broadcast 闁?VMCALL per CPU
// =========================================================================

//
// set up EPT stealth for one page 闁?single VMCALL from current CPU.
// HV internally loops all g_vcpu[i].ept_page_table to split + set PTE.
// NO KeGenericCallDpc 闁?avoids 0x101 CLOCK_WATCHDOG when a CPU is
    ...
    BOOLEAN intercept_write = FALSE)
{
    ...
    // invalid under system CR3 in VMX-root 闁?so we copy the content here.
    //
    ...
    // DPC broadcast 闁?every CPU does VMCALL, each splits its own EPT.
    ...
    BOOLEAN resident)
{
    ...
    while (done < shellcode_size)
    {
        ...
        //
        // page already has content from TdBuildShellcodePage 闁?no need to touch.
        ...
        //
        // NX bit in real PTE is NOT cleared 闁?Windows' MiAgeWorkingSet
