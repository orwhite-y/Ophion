/*
*   hv.h - master hypervisor header 鈥?includes everything needed
*/
#pragma once
...
// the compiler may reorder reads across __writecr3 鈥?use _mm_mfence if needed.
//
...
// different addr -> streak resets), so normal CPUs never write. On a 鍗℃ (freeze)
...
// VMX-root internal 鈥?called from VMCALL handler (guest CR3 must be active)
...
// VMX-root safe EPT 2MB鈫?KB split (uses pre-allocated pool, NOT ExAllocatePool2)
PVMM_EPT_DYNAMIC_SPLIT ept_split_large_page_pool(PVMM_EPT_PAGE_TABLE page_table, SIZE_T phys_addr);

// VMX-root handlers 鈥?called from vmexit dispatch
...
//   read 鈫?clean data (no exec attr), execute 鈫?VMCALL 鈫?handler dispatch
//

// stealth region 鈥?contiguous physical memory for shadow/fake pages
BOOLEAN ept_stealth_region_init(VOID);
VOID    ept_stealth_region_destroy(VOID);

// public API 鈥?PASSIVE_LEVEL
...
// VMX-root handler 鈥?check if VMCALL came from a stealth page
...
// #PF handler 鈥?intercepts instruction-fetch page faults (NX violations)
