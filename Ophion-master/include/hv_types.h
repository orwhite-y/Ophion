...
#define HV_R3_MEM_MAX           131072        // max bytes per R3 read/write call (16 pages, prefetch batch)
...
typedef struct _EPT_HOOK_VMCALL_PARAM {
    ...
    volatile LONG installed;      // [internal] 0�? by first CPU, others just split+PTE
    BOOLEAN result;               // [out]
...
typedef struct _EPT_UNHOOK_VMCALL_PARAM {
    ...
    volatile LONG unhooked;       // [internal] 0�? by first CPU
    BOOLEAN result;               // [out]
...
typedef struct _EPT_STEALTH_ALLOC_PARAM {
    ...
    volatile LONG installed;      // [internal] 0�? by first CPU
    BOOLEAN result;               // [out]
